...
//  AliPush_Demo
//
//  Created by liuxiaobing on 2018/9/13.
...
// appkey & AppSecret
static NSString *const testAppKey = @"25070523";
static NSString *const testAppSecret = @"bfd025c763824e7e4bb1d94837dbcaf9";
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [self registerAPNS:application];         // APNs注册，获取deviceToken并上报
    [self initCloudPush];                    // 初始化SDK
    [self listenerOnChannelOpened];          // 监听推送通道打开动作
    [self registerMessageReceive];           // 监听推送消息到达
    [CloudPushSDK sendNotificationAck:launchOptions];       // 点击通知将 App 从关闭状态启动时，将通知打开回执上报 // 计算点击 OpenCount
    
    [self getNotificationSettingStatus];     // 主动获取设备通知是否授权 (iOS 10+)
    ...
    if (launchOptions) {
        NSLog(@"86--- ====== launchOptions: %@",launchOptions);
...
- (void)registerAPNS:(UIApplication *)application {
    ...
    if (systemVersionNum >= 10.0) {
        _notificationCenter = [UNUserNotificationCenter currentNotificationCenter];     // iOS 10 notifications
        [self createCustomNotificationCategory];        // 创建 category，并注册到通知中心
        ...
        [_notificationCenter requestAuthorizationWithOptions:UNAuthorizationOptionAlert | UNAuthorizationOptionBadge | UNAuthorizationOptionSound completionHandler:^(BOOL granted, NSError * _Nullable error) {
            ...
            if (granted) {
                NSLog(@"109 ---- User authored notification.");
                ...
                NSLog(@"116 --- User denied notification.");
                // 即使客户不允许通知也想让它通知 // 待测试
                // 向APNs注册，获取deviceToken  // 要求在主线程中
                dispatch_async(dispatch_get_main_queue(), ^{
                    [application registerForRemoteNotifications];
...
- (void)onMessageReceived:(NSNotification *)notification {
    NSLog(@"304------ ====== Receive one message !!!!!!!");
    CCPSysMessage *message = [notification object];
    NSString *title = [[NSString alloc] initWithData:message.title encoding:NSUTF8StringEncoding];
    NSString *body = [[NSString alloc] initWithData:message.body encoding:NSUTF8StringEncoding];
    NSLog(@"308----- Receive message title: %@, content: %@.", title, body);
    NSLog(@"309------ 当前线程 %@",[NSThread currentThread]);
